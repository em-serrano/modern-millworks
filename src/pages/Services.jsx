...
const services = [
  {
    ...
    description:
      'Precision installation of premium European kitchen and cabinet systems from world-renowned Italian and German manufacturers. Our decades of experience ensure flawless installation and seamless integration with your architectural elements, guaranteeing perfect alignment, superior craftsmanship, and exceptional results that showcase the prestigious quality these luxury brands demand.',
    iconPath: (
      <>
        <path
          strokeLinecap="round"
...
const Icon = ({ children }) => (
  <svg
    className="w-12 h-12 text-blue-700 mb-4 transition-all duration-300 group-hover:text-white relative z-10"
    fill="none"
    stroke="currentColor"
    viewBox="0 0 24 24"
  >
...
export default function Services() {
  ...
  const renderServiceCard = (service, index, isMobile = false) => (
    <div
      key={index}
      className="group relative overflow-hidden transition-all duration-500 border border-gray-200 rounded-lg p-6 bg-white shadow-lg hover:shadow-2xl transform hover:-translate-y-1"
    >
      ...
      <div className="relative z-10">
        <div
          className={`flex ${isMobile ? 'items-center mb-4' : 'justify-center mb-6'}`}
        >
          <Icon>{service.iconPath}</Icon>
          <h3 className="text-2xl font-semibold text-blue-800 group-hover:text-white transition-colors duration-300 ml-2">
            {service.title}
          </h3>
        </div>
        <p className="text-gray-600 group-hover:text-gray-100 leading-relaxed text-lg transition-colors duration-300">
          {service.description}
        </p>
      </div>
    </div>
      ...
      <div className={`${isMobile ? 'text-center' : 'grid md:grid-cols-3 gap-8 items-center'}`}>
        <div className={isMobile ? '' : 'md:col-span-1'}>
          ...
          <p className="text-gray-600 leading-relaxed text-lg mb-6">
            Explore our portfolio of completed luxury installations showcasing
            the finest European brands and craftsmanship.
          </p>

          {/* ✅ Updated to Link */}
          <Link
            to="/projects"
            className="inline-flex items-center text-blue-700 font-medium text-lg group-hover:text-blue-500"
          >
            View All Projects
            <svg
  ...
  return (
    <>
      ...
      <section
        id="services"
        className="relative w-full py-16 md:py-8 px-4 sm:px-6 md:px-12 bg-white grid-background font-[Montserrat]"
      >
